...
int main(int argc, char **argv)
{
    ...
    PointCloudT::Ptr cloud_out_side(new PointCloudT);
    PointCloudT::Ptr cloud_out_plane(new PointCloudT);
    PointCloudT::Ptr cloud_out_plane_model(new PointCloudT);
    PointCloudT::Ptr cloud_nor(new PointCloudT);
    pcl::io::loadPCDFile("../data/result/charger_process/charger_out_filter.pcd", *cloud_nor);
    ...
    ne.setRadiusSearch(0.005); // 设置半径领域的大小
    ...
    int nor_size_1 = 0;
    int theta_threshold;
    theta_threshold = atoi(argv[1]);
    for (size_t i = 0; i < cloud_normal->size(); i++)
    {   
        Eigen::Vector3d z_axis(0, 0, 1);
        Eigen::Vector3d param_normal(cloud_normal->points[i].normal_x,cloud_normal->points[i].normal_y, cloud_normal->points[i].normal_z);

    // 法向量和z轴叉乘得到旋转向量
        Eigen::Vector3d rotation_axis = z_axis.cross(param_normal).normalized();

        double theta = acos(z_axis.dot(param_normal) / (z_axis.norm() * param_normal.norm()));
        ...
        // else
        // {
        //     if (theta * 180 / 3.1415 > 30)
        //     {
        //         continue;
        //     }
        // }
        // std::cout << "normal_x:" << cloud_normal->points[i].normal_x << "  normal_y:" << cloud_normal->points[i].normal_y << "   normal_z:" << cloud_normal->points[i].normal_z << std::endl;
        std::cout << "theta [ "<<i<<"] = "<<theta * 180 / 3.1415 << std::endl;
        if (theta * 180 / 3.1415 > theta_threshold){
            cloud_out_side->points.push_back(cloud_nor->points[i]);
        }
        else
        {
            cloud_out_plane->points.push_back(cloud_nor->points[i]);
        }
    ...
    if (show_nor)
    {
        ...
        // viewer.addPointCloud<PointT>(cloud_nor, "original_cloud");
        // viewer.addPointCloud<PointT>(cloud_out_side, "cloud_out_side");
        // viewer.addPointCloud<PointT>(cloud_out_plane, "cloud_out_side");
        pcl::visualization::PointCloudColorHandlerCustom<PointT> color(cloud_out_side, 0, 255, 0);
        pcl::visualization::PointCloudColorHandlerCustom<PointT> color_plane(cloud_out_plane, 255, 0, 0);
        viewer.addPointCloud<PointT>(cloud_out_side, color, "cloud_out_side");
        viewer.addPointCloud<PointT>(cloud_out_plane, color_plane, "cloud_out_plane");
        ...
        // viewer.addPointCloudNormals<PointT, pcl::Normal>(cloud_nor, cloud_normal, 1, 0.005, "normal");
        while (!viewer.wasStopped())
        {
            viewer.spinOnce(300);
        }
    }
    cloud_out_side->width=1;
    cloud_out_plane->width = 1;
    cloud_out_side->height=cloud_out_side->size();
    cloud_out_plane->height=cloud_out_plane->size();
    pcl::io::savePCDFile("../data/result/normal_estimation/cloud_out_side.pcd",*cloud_out_side);
    pcl::io::savePCDFile("../data/result/normal_estimation/cloud_out_plane.pcd",*cloud_out_plane);
    for(size_t i=0;i<cloud_out_plane->size();i++){
        if(cloud_out_plane->points[i].z>0.02){

        }
        else{
            cloud_out_plane_model->push_back(cloud_out_plane->points[i]);
        }
    }
    pcl::io::savePCDFile("../data/result/normal_estimation/cloud_out_plane_model.pcd",*cloud_out_plane_model);
